<div id="comments">
    <h2>Всего комментариев: <?= get_comments_number(); ?></h2>
    <?php if (have_comments()) : ?>
        <ul class="comment-list media-list">
            <?php wp_list_comments(array('walker' => new CleanCommentsWalker())); ?>
        </ul>
        <?php if (get_comment_pages_count() > 1 && get_option('page_comments')) :
            ...
            if (is_array($page_links)) :
                ...
                foreach ($page_links as $link):
                    echo strpos($link, 'current') !== false ? "<li class='active'>$link</li>" : "<li>$link</li>";
                endforeach;
                echo '</ul>';
            endif;
